...
casper.test.begin('VFB control panel tests', 9, function suite(test) {
    ...
    casper.start(PROJECT_URL, function () {
        ...
        this.waitForSelector('#VFB_00000001_deselect_buttonBar_btn', function () {
            this.echo("I waited for the logo and VFB_00000001 deselect button on button bar to load.");
    ...
    casper.then(function () {
        test.assertExists('div#VFBTermInfo_el_2_component', 'Term info correctly populated  for neuron after load');
        test.assertExists('button[id=VFB_00000001_zoom_buttonBar_btn]', 'Term info button bar button created');
        test.assertTextExists('fru-M-200266 (VFB_00000001)', 'Term info correctly populated at startup');
    });
    ...
    casper.then(function () {
        ...
        casper.wait(1000, function () {
            ...
            casper.waitForSelector('#VFB_00017894_deselect_buttonBar_btn', function () {
                casper.wait(1000, function () {
                    test.assertExists('div#VFBTermInfo_el_1_component', 'Term info correctly populated  for JFRC2_template after control panel selection click');
